...
def prompt2conversation(prompt, tokenizer,modelname):
        ...
        if "OLMoTokenizerFast" in str(type(tokenizer)):
            chat = [{"role": "user", "content": prompt}]
            return tokenizer.apply_chat_template(chat, tokenize=False, add_generation_prompt=True)
        else:
            msg = prompt
            conv = get_conversation_template(modelname)
...
def run_inference(model_inputs,item_list,args):
    ...
    if args.engine == "vllm": 
        todo_inputs = [prompt2conversation(prompt, tokenizer,args.model_name) for prompt in model_inputs]
